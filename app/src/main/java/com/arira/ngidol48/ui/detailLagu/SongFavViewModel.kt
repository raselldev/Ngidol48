...
import com.arira.ngidol48.repo.FavoritRepo
import com.arira.ngidol48.repo.SongRepo
import io.reactivex.android.schedulers.AndroidSchedulers
import io.reactivex.disposables.Disposable
import io.reactivex.schedulers.Schedulers

class SongFavViewModel : ViewModel() {

    private lateinit var subscription: Disposable
    var context: Context? = null
    var repo = FavoritRepo()
    val data = MutableLiveData<FavoritResponse>()
    ...
    fun getResponse(): LiveData<FavoritResponse> {
        return data
    }

    fun fav(idLagu:String) {
        subscription = repo.add(idLagu)
    ...
    fun remove(idLagu:String) {
        subscription = repo.remove(idLagu)
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .doOnSubscribe { isLoading.value = true }
            .doOnTerminate { isLoading.value = false }
